...
void   DP_LTL(Node **subformula, HyDis **MonitorTable, FWTaliroParam *p_par, int phi_size, int i,int g){
	...
	for (k = phi_size; k >= 1; k--){
		...
		case NOT:
            ...
            MonitorTable[k][i].iteration = MonitorTable[subformula[k]->Lindex][i].iteration;
            MonitorTable[k][i].preindex = MonitorTable[subformula[k]->Lindex][i].preindex;
...
void Resolve_Data(int currentSample,int totalSample,int g,Node **subformula, HyDis **MonitorTable, int phi_size){
    int k,kk,index[2];
    if(g>1)
        return;
    //mexPrintf("Resolve_Data %d\n",currentSample);
    for (k = phi_size; k >= 1; k--){
 		if (subformula[k]->ntyp==DATA)
 		{
//            mexPrintf("%d->",currentSample);
            index[0]=currentSample;
            for( kk = currentSample ; kk < totalSample ; kk++){
//                 mexPrintf("%d|",kk);
                 index[1]=kk;
                 MonitorTable[k][kk].ds=SignedDist2(index, subformula[k]->sym->set, 2);
                 if (MonitorTable[k][kk].ds>100000)
                     MonitorTable[k][kk] = SetToInf(+1, currentSample);  
                 else if (MonitorTable[k][kk].ds<-100000)
                     MonitorTable[k][kk] = SetToInf(-1, currentSample);  
                 else{
                    MonitorTable[k][kk].dl=0;
                    MonitorTable[k][kk].iteration = currentSample;
                    MonitorTable[k][kk].preindex = subformula[k]->sym->index;
                 }
            }
//            mexPrintf("\n");
        }
...
//     double infval = mxGetInf();
	...
	for (iii = 1; iii <= phi_size; iii++)			/*	check the index for subformula*/
	{
        ...
        //else                         mexPrintf("%d->OP_%d\n",iii,subformula[iii]->ntyp);
        if (subformula[iii]->ntyp==PREDICATE) 
			if (strcmp(subformula[iii]->sym->name, "data") == 0){
				subformula[iii]->ntyp = DATA;
				mexPrintf("data\n");
			}
	}
                ...
                else  if ((p_par->nInp==6) && (subformula[iii]->sym->set->nloc==0) && (p_par->nCLG>=1)) //used for multiple H.A.s
                {
                    ...
                    for(i=0;i<p_par->nCLG;i++){
                        if (subformula[iii]->sym->set->nlocNCLG[i] == 0){
                            robustness.ds = SignedDist(sysTraj, subformula[iii]->sym->set, p_par->SysDim);
                            robustness.dl = 0;
            			}
                		else
                    	{
                        	robustness = SignedHDist0NCLG(sysTraj, subformula[iii]->sym->set, p_par->SysDim, p_distData->LDistNCLG, p_par->tnLocNCLG[i], i);
                        }
                        robHA[i]=robustness;
                    }
                    ...
                    MonitorTable[iii][ii].ds=robustness.ds;
                    MonitorTable[iii][ii].dl=robustness.dl;
                    MonitorTable[iii][ii].iteration = ii;
            		MonitorTable[iii][ii].preindex = subformula[iii]->sym->index;
            		if (subformula[iii]->sym->Normalized)
                    {
                        MonitorTable[iii][ii] = NormalizeHybrid(MonitorTable[iii][ii], subformula[iii]->sym->NormBounds[1], subformula[iii]->sym->NormBounds[0]);
            		}
                }
                else
                {
	...
	if (p_par->LTL==1){
        ...
        for (ii = maxTimeVariable ; ii >=1 ; ii--)		/* time stamps for DP */
		{
            for (jj = 0; jj < p_par->nSamp ; jj++)			/* time stamps for DP*/
        	{
                ...
                Resolve_Data(jj,p_par->nSamp,ii,subformula,MonitorTable,phi_size);
                //mexErrMsgTxt("mx_tq_taliro: Data Found!\n");
                for (kk = p_par->nSamp -1 ; kk >=jj ; kk--)
